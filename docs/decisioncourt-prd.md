...
> 版本：v0.5+
> 状态：v0.5 增补 MemoryBus 统一（Episodic Memory via A2A）+ ContextView 投影 + ReAct 私有策略自动分类 + 前端 MemoryAuditPanel；v0.5+ 修补 SessionUUID 房间钥匙 bug + MemoryEntry 结构化字段（立场 / 置信度 / 推理）+ AgentAvatar 思考脉冲动画
> 目标：为复杂人生/工作决策提供一个结构化、可审计、人机协作的多 Agent 辩论法庭。
> 设计演进参见：[memory-a2a-redesign.md](../.trae/documents/memory-a2a-redesign.md)
...
#### 4.3.2 信念引擎（Belief Engine）

每个 Agent 维护对两个选项的信念度 `belief_A` 和 `belief_B`，满足 `belief_A + belief_B = 1`。

| Agent | 初始信念度 | 更新规则 |
|---|---|---|
| 控方 | `belief_A ≥ 0.7` | 支持 option A 的证据 → 提升 `belief_A` |
| 辩方 | `belief_A ≤ 0.3` | 支持 option A 的证据 → 降低 `belief_A` |
| 调查员 | `belief_A = 0.5` | 根据搜索结果动态调整 |
| 书记员 | `belief_A = 0.5` | 保持中立，不参与对抗 |

**强制立场一致性**：
- Agent 生成发言前，系统先检查其当前信念度。
- 如果发言内容与信念度方向不一致（例如控方在 `belief_A < 0.5` 时仍在支持 option A），LLM-as-judge 会打回重生成。
- 发言中必须引用至少一条证据，不能空泛表态。

**智能收敛判断**：
- 每轮质证后，计算本轮信念度变化 `Δ = |belief_current - belief_previous|`。
- 如果连续两轮 `Δ < 5%`，认为辩论已经收敛，提前触发判决阶段。

#### 4.3.3 防止诡辩与重复

- 限制每轮发言长度（最多 300 字）。
- 禁止引用已经被反驳且未翻盘的证据。
- 引入"新意度"检查：如果 Agent 本轮发言与之前轮次重复度超过 60%，要求其更换角度。
...
**必须做：**
- [ ] 通用决策模板：用户可自定义问题和两个选项
- [ ] 问题澄清与选项生成：用户只给模糊问题时，Agent 澄清并生成候选选项
- [ ] 庭审模式选择：快速（2 轮）/ 标准（3 轮，默认）/ 深度（5 轮）
- [ ] 智能收敛：信念度连续两轮变化 < 5% 时提前结束庭审
- [ ] 4 个核心 Agent：控方、辩方、调查员、书记员
- [ ] 信念引擎：每个 Agent 维护并更新对选项的信念度
- [ ] 用户提交证据
- [ ] Agent 主动提问：识别信息缺口后向用户提问，回答转为证据
- [ ] 调查员 Agent 做简单 WebSearch（开发用免费，部署用 Tavily）
- [ ] 基础庭审流程：开庭 → 举证 → 质证 → 调查补充 → 结案 → 判决
- [ ] 观点地图 + 立场变化曲线
- [ ] 判决书生成与导出
- [ ] Docker Compose 一键启动
...
| 智能收敛 | ✅ | `service.go:559 isConverged` 实现，trial.converged 事件已广播 |
...
1. **PostgreSQL 未运行**：后端 `DATABASE_URL` 无法连接，服务起不来。
2. **Docker 未安装**：无法使用 `docker-compose` 验证完整环境。
3. **建议**：先安装 Docker Desktop（含 PostgreSQL）或本地启动 PostgreSQL，再联调后端。
