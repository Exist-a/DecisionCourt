...
- **DecisionCourt 含义**：进程内事件总线，**A2A 总线模式**（5 个 Agent 共享消息）

**答面试要小心区分**。

### A2A Bus（DecisionCourt 含义）
...
### Court Session（庭审会话）
1 次庭审 = 1 个 `court_sessions` row + N 个 evidence + N 个 belief_diff + N 条 A2A message + 最终 1 个 verdict。
