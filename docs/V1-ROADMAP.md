...
> **版本**：v1.0.0 (2026-08-20 落地) + v1.0.x 系列补丁 + 远期 v2.0
...
> 2. **v1.0.5 重新规划**（原剪影小人 SVG 方案作废，用户重新定方向）
> 3. **v1.2 安全 P1 触发后再启动**（不变，但实际优先级低）
> 4. **v2.0 不再绑定"商业化稳定 6 个月"** — 个人项目视角
> **目标**：从"产品级稳定(v1.0.0)"持续打磨到"个人长期维护的产品"，按用户实际反馈动态调整里程碑
...
## 3. 技术依赖关系（2026-08-21 重规划）

```
M0 v1.0.0 ✅ ─┐
M0.5 v1.0.1 ✅│
M1 v1.0.2 ✅ ─┴─ M2 v1.0.3 ( PR-B1 ✅ / PR-B2 + B3 ⏸ ) ── M3 v1.0.4 ⏸ ── M4 v1.0.5 ⏸ (重新规划) ── v1.0.6+ ⏸
                                    │
                                    └── v1.2 ⏸ (安全 P1, 触发后启动) ── v2.0 ⏸ (远期)
```

- **v1.0.3 PR-B2/B3** → v1.0.4 顺序依赖（PR-B2 完成后启动 v1.0.4）
- **v1.0.4** → v1.0.5 顺序依赖（Trace + Framer Motion 给 v1.0.5 提供数据/动画基础设施）
- **v1.0.x** 与 v1.2 / v2.0 无强依赖（三线独立：产品打磨 / 安全 / 远期）
...
| **M4 v1.0.5** | v1.0.4 后 | **重新规划中**（原剪影小人方案作废，用户重新定方向）| ⏸ |
| M5 v1.0.6+ | v1.0.5 后 | 持续打磨（议题池 + 用户反馈驱动）| ⏸ |
| M6 v1.2 | 公网部署 / 安全事件 / 个人决定 | D1 安全 P1 全部落地 | ⏸ |
| M7 v2.0 | 远期 | 多模态 + 实时协作 + AI 法官（个人项目视角）| ⏸ |
...
4. **v1.0.5 重新规划**——用户须先告知新方向。原剪影小人 SVG 方案作废（用户 2026-08-21 反馈）

### 长期（按需触发）

5. **v1.2 安全 P1** — 触发条件：公网部署 / 安全事件 / 用户决定继续安全加固
6. **v2.0 多模态 + 实时协作 + AI 法官** — 个人项目远期参考（不再绑定"商业化稳定 6 个月"）
