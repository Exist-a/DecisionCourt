...
type AgentGatewayConfig struct {
	...
	// SmartCompressionAbstractiveSummary (v2.10 ADR 0044 #6): 用轻量 LLM 生成
	// 保留推理链的 abstractive 摘要。默认 false —— 每次 trial 多一次 LLM 调用，
	// 属成本 opt-in。开启后 LLM 失败自动回退 extractive。
	SmartCompressionAbstractiveSummary bool `mapstructure:"AGENT_GATEWAY_SMART_COMPRESSION_ABSTRACTIVE_SUMMARY"`
...
SmartCompression:       envOrDefaultBool("AGENT_GATEWAY_SMART_COMPRESSION", true),
	...
	fromEnv := 0
	for _, key := range diagnosticEnvKeys {
		if _, ok := os.LookupEnv(key); ok {
			fromEnv++
		}
	}
	// total 直接取自列表长度, 不再手写常量 —— v2.10 之前是硬编码 43,
...
// 新增 env 时把它加进来即可（total 由 len() 自动得出，无需改别处）。
var diagnosticEnvKeys = []string{
	...
	"AGENT_GATEWAY_SMART_COMPRESSION_ABSTRACTIVE_SUMMARY",
	...
	"AGENT_GATEWAY_FILE_LOGGER_PROMPTS", "AGENT_GATEWAY_FILE_LOGGER_PROMPTS_MAX_BYTES",
}
